...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "crunchdev", "tests")); system.time(test_check("crunchdev", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-export _R_CHECK_CRAN_INCOMING_REMOTE_=FALSE && R CMD check --as-cran crunchdev_$(VERSION).tar.gz
	rm -rf crunchdev.Rcheck/
